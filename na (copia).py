...
def pendulo (init, t):
    ...
    dv_x= ((m*dw*dw*l*sin(w)-m*dv_w*l*cos(w))/(m+M) ) #aceleracion en x
...
scene.range = 2 #tamaño del sistema 
...
sleeptime = 0.001 #tiempo entre cada intervalo 
prtcl = sphere(pos=vector(x,y,z), radius=0.1, color=color.cyan) #referente ala esfera donde se ubica
...
while t_run < t_final:
    sleep(sleeptime)
    prtcl.pos = vector( xx[time_i]+l*sin(ww[time_i]), -l*cos(ww[time_i]), z ) #actualiza la posición de la esfera 
    prtcl2.pos = vector( xx[time_i],y2, z )
    t_run += t_delta #aumemta para evaluar en el tiempo 
    time_i += 1
